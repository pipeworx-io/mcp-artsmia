...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Minneapolis Institute of Art');
}
...
async function esSearch(query: string, size: number): Promise<EsResponse> {
  const res = await pwFetch(`${BASE}/${encodeURIComponent(query)}?size=${size}`, {
...
async function getArtwork(args: Record<string, unknown>): Promise<unknown> {
  ...
  const res = await pwFetch(`${BASE}/id/${id}`, {
    headers: { Accept: 'application/json', 'User-Agent': UA },
  });
